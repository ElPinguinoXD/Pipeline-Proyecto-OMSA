...
import unittest
from maquina_cafe import MaquinadeCafe
...
class TestMaquinaCafe(unittest.TestCase):
    ...
    def test_mostrar_mensaje_faltante(self):
        self.assertEqual(self.maquina.verificar_ingredientes(False, True, True), "No hay vasos")
        self.assertEqual(self.maquina.verificar_ingredientes(True, False, True), "No hay azucar")
        self.assertEqual(self.maquina.verificar_ingredientes(True, True, False), "No hay café")
